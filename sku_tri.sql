SELECT DISTINCT
                ...
                FROM catalog_product_link l
                    JOIN (
                    SELECT c.* , (
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, 'AFA', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, 'FA6', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, 'A67', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, '675', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, '750', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, '50-', ''))) / 3 + 
			(LENGTH(c.body) - LENGTH(REPLACE(c.body, '0-M', ''))) / 3) AS match_score
                    FROM (
                        SELECT child.entity_id, child.sku, cname.value AS name, cname.store_id AS store_id,LOWER(SUBSTRING_INDEX(sku, '-', 1)) AS prefix,LOWER(SUBSTRING(sku, LOCATE('-', sku) + 1, LENGTH(sku))) AS body

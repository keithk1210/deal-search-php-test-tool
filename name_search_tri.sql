SELECT DISTINCT
                ...
                FROM catalog_product_link l
                    JOIN (
                    SELECT c.*, ((LENGTH(LOWER(c.name)) - LENGTH(REPLACE(LOWER(c.name), 'oli', ''))) / 3 + (LENGTH(LOWER(c.name)) - LENGTH(REPLACE(LOWER(c.name), 'liv', ''))) / 3 + (LENGTH(LOWER(c.name)) - LENGTH(REPLACE(LOWER(c.name), 'iva', ''))) / 3) AS match_score
                    FROM (
